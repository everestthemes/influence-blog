...
var RepeaterRow = function( rowIndex, container, label ){
	...
	this.label      = label;
	this.header     = this.container.find( '.repeater-one-row-header' ),

	this.header.on( 'click', function() {
		self.toggleMinimize();
	});

	this.container.on( 'click', '.repeater-one-row-remove', function() {
		self.remove();
	});
	...
	this.updateLabel = function() {
		...
		if ( 'field' === this.label.type ) {
			...
			if ( 'function' === typeof rowLabelField.val ) {
				...
				if ( '' !== rowLabel ) {
					this.header.find( '.repeater-one-row-label' ).text( rowLabel );
					return;
				}
			}
		}
		this.header.find( '.repeater-one-row-label' ).text( this.label.value + ' ' + ( this.rowIndex + 1 ) );
	};
...
wp.customize.controlConstructor.repeater = wp.customize.Control.extend({
	ready: function(){
		...
		this.repeaterFieldsContainer = this.container.find( '.repeater-one-fields' ).first();
		...
		this.container.on( 'click', 'button.repeater-one-add', function( e ) {
			e.preventDefault();
		...
		this.container.on( 'click', '.repeater-one-row-remove', function( e ) {
		...
		this.container.on( 'click keypress', '.repeater-one-field-image .upload-button,.repeater-one-field-cropped_image .upload-button,.repeater-one-field-upload .upload-button', function( e ) {
			e.preventDefault();
			control.$thisButton = jQuery( this );
			control.openFrame( e );
		});

		this.container.on( 'click keypress', '.repeater-one-field-image .remove-button,.repeater-one-field-cropped_image .remove-button', function( e ) {
			e.preventDefault();
			control.$thisButton = jQuery( this );
			control.removeImage( e );
		});

		this.container.on( 'click keypress', '.repeater-one-field-upload .remove-button', function( e ) {
			e.preventDefault();
			control.$thisButton = jQuery( this );
		...
		this.repeaterFieldsContainer.sortable({
			handle: '.repeater-one-row-header',
	...
	openFrame: function( event ){
		...
		if ( this.$thisButton.closest( '.repeater-field' ).hasClass( 'repeater-one-field-cropped_image' ) ) {
	...
	onSelect: function(){
		...
		if ( this.$thisButton.closest( '.repeater-field' ).hasClass( 'repeater-one-field-upload' ) ) {
	...
	setImageInRepeaterField: function( attachment ){
		...
		var $targetDiv = this.$thisButton.closest( '.repeater-one-field-image,.repeater-one-field-cropped_image' );

		$targetDiv.find( '.repeater-one-image-attachment' ).html( '<img src="' + attachment.url + '">' ).hide().slideDown( 'slow' );
	...
	setFileInRepeaterField: function( attachment ){
		...
		var $targetDiv = this.$thisButton.closest( '.repeater-one-field-upload' );

		$targetDiv.find( '.repeater-one-file-attachment' ).html( '<span class="file"><span class="dashicons dashicons-media-default"></span> ' + attachment.filename + '</span>' ).hide().slideDown( 'slow' );
	...
	removeImage: function( event ){
		...
		$targetDiv = this.$thisButton.closest( '.repeater-one-field-image,.repeater-one-field-cropped_image,.repeater-one-field-upload' );
		$uploadButton = $targetDiv.find( '.upload-button' );

		$targetDiv.find( '.repeater-one-image-attachment' ).slideUp( 'fast', function() {
	...
	removeFile: function( event ){
		...
		$targetDiv = this.$thisButton.closest( '.repeater-one-field-upload' );
		$uploadButton = $targetDiv.find( '.upload-button' );

		$targetDiv.find( '.repeater-one-file-attachment' ).slideUp( 'fast', function() {
			jQuery( this ).show().html( jQuery( this ).data( 'placeholder' ) );
		});
		$targetDiv.find( '.hidden-field' ).val( '' );
		$uploadButton.text( $uploadButton.data( 'label' ) );
...
jQuery( document ).ready(function($){
    'use strict';
    $(document).on('click','.repeater-one-add',function() {
