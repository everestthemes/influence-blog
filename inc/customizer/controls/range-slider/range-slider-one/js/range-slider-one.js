wp.customize.controlConstructor['range-slider-one'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
			...
			input_default,
			changeAction,
			range_slider_one_input_number_timeout;

		// Update the text value
		jQuery( 'input[type=range]' ).on( 'mousedown', function() {

			range 			= jQuery( this );
			range_input 	= range.parent().children( '.range-slider-one-input' );
		...
		function range_slider_one_autocorrect_range_input_number( input_number, timeout ) {
			...
			clearTimeout( range_slider_one_input_number_timeout );

			range_slider_one_input_number_timeout = setTimeout( function() {
		...
		jQuery( 'input.range-slider-one-input' ).on( 'change keyup', function() {

			range_slider_one_autocorrect_range_input_number( jQuery( this ), 1000);

		} ).on( 'focusout', function() {

			range_slider_one_autocorrect_range_input_number( jQuery( this ), 0);

		} );

		// Handle the reset button
		jQuery( '.range-slider-one-reset-slider' ).on('click', function() {
